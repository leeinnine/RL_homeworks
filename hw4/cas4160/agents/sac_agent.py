...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            min_q = torch.min(next_qs, 0).values
            next_qs = min_q.expand_as(next_qs)
            # raise NotImplementedError
        else:
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs_expanded = obs.unsqueeze(0).expand(self.num_actor_samples, batch_size, *obs.shape[1:])
            q_values = self.critic(
                obs_expanded.reshape(-1, *obs.shape[1:]),
    ...
    def update(
        ...
        if self.target_update_period is not None:   # hard update
            if step % self.target_update_period == 0:
                self.update_target_critic()
        elif self.soft_target_update_rate is not None:  # soft update
